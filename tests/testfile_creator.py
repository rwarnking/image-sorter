    ...
    for i, format in enumerate(lst_formats):
        ...
        for j, date in enumerate(lst_dates):
            ...
            for ext in lst_endings:
                out_dir = OUT_DIRS[j]
                shutil.copy2(
                    join(IN_DIR, f"sample{ext}"),
                    join(out_dir, f"{file_date}{ext}")
                )
...
def modify_metadata_piexif(file_with_path, data, date):
    """
    ...
    exif_dict["0th"][piexif.ImageIFD.Artist] = "Test" if data["Artist"] else ""
    ...
    if data["Date"]:
        ...
        # Convert to binary
        date_str = date_str.encode("ascii")
        exif_dict["Exif"][piexif.ExifIFD.DateTimeOriginal] = date_str

        # Add millisecounds
        ms_str = date.strftime("%Y:%m:%d %H:%M:%S.%f")[-6:-3]
        ms_str = ms_str.encode("ascii")
        exif_dict["Exif"][piexif.ExifIFD.SubSecTimeOriginal] = ms_str
